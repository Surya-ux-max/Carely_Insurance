      ...
      <div className="hidden lg:flex lg:w-1/2 bg-red-600 flex-col justify-between p-14 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-5xl font-black text-white leading-tight tracking-tight mb-6">
            Income protection<br />for every gig<br />worker.
          </h2>
          <p className="text-red-200 text-lg font-light leading-relaxed max-w-sm">
            Automatic payouts when disruptions hit. No paperwork, no waiting — just instant support.
          </p>
        </div>

        <div className="relative z-10 space-y-4">
          {[
            { icon: FaMotorcycle,  title: 'Gig Workers',  desc: 'Subscribe, get covered, get paid automatically' },
            { icon: FaUserShield,  title: 'Admins',       desc: 'Monitor zones, claims, fraud and payouts live'  },
          ].map((item, i) => (
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex bg-gray-100 rounded-2xl p-1 mb-8">
            {([['worker', FaMotorcycle, 'Worker'], ['admin', FaUserShield, 'Admin']] as const).map(([t, Icon, label]) => (
              <button
                key={t}
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all duration-200 ${
                  tab === t ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
          ...
          {tab === 'worker' && (
            <div className="space-y-4">
              <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                <p className="text-gray-400 text-[11px] font-semibold mb-1">Demo credentials</p>
